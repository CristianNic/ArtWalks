...
const id="LiverpoolTheme_wt74_block_wtMainContent_WebPatterns_wt165_block_wtTitle_WebPatterns_wtWebScreenTitleDesktop_block_wtContent"
...
async function compileJSON() {
  ...
  for (let i = 0; i < publicArt.length; i++) {
    ...
    const output = {
      registryID: publicArt[i].registryID,
      title: title.toString(),
      artists_id: publicArt[i].artists,
      ...
      installation_year: publicArt[i].installation_year,
    }
    emptyArr.push(output)
  }
  fs.writeFile('csv-json-public-art-final.js', JSON.stringify(emptyArr), (err) => {
  if (err) throw err;
